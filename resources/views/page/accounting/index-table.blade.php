...
<table class="display table table-bordered table-hover accounting-table">
    ...
    <tr>
        ...
        <th class="text-center sticky-col sticky-col-5" rowspan="2">Lương cơ bản</th>
        <th colspan="4" class="text-center">Hệ số lương cơ bản</th>
    ...
    @if ( $hasPayroll )
        @foreach($employees as $index => $employee)
            ...
            <tr>
                ...
                <td class="text-center sticky-col sticky-col-5">{{ $employee->salary_gross ? number_format($employee->salary_gross) : 0 }}</td>
                ...
                @foreach( $allowances->groupBy('type') as $type => $group)
                    ...
                    @endphp
                    <td class="text-end">{{ number_format($rate * $employee->salary_gross) }}</td>
                @endforeach
                @php
                    $totalAllowance = $allowances->groupBy('type')->sum(function($group) use ($employee, $payroll) {
                        ...
                        return $rate * ($employee->salary_gross ?? 0);
                    });
